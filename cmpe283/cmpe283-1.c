...
/*
 * Processor-Based VM-Execution Control capabilities
...
struct capability_info procbased2[23] =
{
	...
	{ 22, "Mode-based execute controls for EPT" },
	{ 25, "Use TSC scaling" }
};

/*
 * Exit VM-Execution Control capabilities
 * See SDM volume 3, section 24.6.2
 */
...
/*
 * Entry VM-Execution Control capabilities
 * See SDM volume 3, section 24.6.2
 */
struct capability_info entry_controls[9] =
{
	...
	{ 17, "Conceal VM exits from Intel PT" }
};
...
detect_vmx_features(void)
{
	...
	//pr_info("Basic Controls MSR 23rd bit: 0x%llx\n",(uint64_t)hi & (1<<23));	
	if((uint64_t) (hi & (1<<23))){
		...
		pr_info("True Procbased Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		//pr_info("True Procbased Controls MSR Shifted: 0x%llx\n",(uint64_t)((uint64_t)lo>>31 | (uint64_t)hi >> 31));
